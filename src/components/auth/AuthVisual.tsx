...
const CARDS = [
  ...
  },
  {
    ...
    progress: 100,
    members: ["R", "L", "P", "D"],
    memberColors: ["bg-indigo-400", "bg-amber-400", "bg-sky-400", "bg-rose-400"],
  },
  {
    id: 4,
    tag: "Backlog",
...
const SlideCard = ({
  ...
  >
    <div className="w-full max-w-sm bg-white/[0.1] backdrop-blur-md rounded-2xl p-6 border border-white/[0.12] shadow-2xl">
      {/* Tag */}
        ...
        <div className="w-full h-1.5 rounded-full bg-white/10 overflow-hidden">
          <motion.div
            ...
            className={`h-full rounded-full ${
              card.progress === 100 ? "bg-emerald-400" : "bg-indigo-400"
            }`}
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        <div className="flex -space-x-2">
          {card.members.map((m, i) => (
            <div
              key={i}
              className={`w-7 h-7 rounded-full ${card.memberColors[i]} flex items-center justify-center text-white text-xs font-bold border-2 border-indigo-700/50`}
            >
              {m}
            </div>
          ))}
...
const AuthVisual = () => {
  ...
  return (
    <div className="hidden lg:flex lg:w-[55%] relative overflow-hidden bg-gradient-to-br from-indigo-600 via-violet-600 to-purple-700">
      ...
      {/* Glow orbs */}
      <motion.div
        className="absolute top-[18%] left-[12%] w-80 h-80 rounded-full blur-3xl bg-indigo-400/25"
        animate={{ scale: [1, 1.15, 1], opacity: [0.25, 0.35, 0.25] }}
        transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="absolute bottom-[18%] right-[8%] w-72 h-72 rounded-full blur-3xl bg-purple-400/20"
      ...
      >
        ...
        <div>
          ...
          <p className="text-white/45 text-xs">Manage tasks that matter</p>
        </div>
      </motion.div>
    </div>
  );
